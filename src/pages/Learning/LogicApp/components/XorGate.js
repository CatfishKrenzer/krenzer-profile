...
const lightGate = require('../gate-images/xor-light.png');
const darkGate = require('../gate-images/xor-dark.png');
...
const XorGate = (props) => {
  ...
  return (
    <>
      <h2>XOR Gate Logic</h2>
      <p>
        An XOR gate (eXclusive OR) will provide a true output if and ONE input is true, but NOT if none or multiple are true.
      </p>
      <h3>Truth Table</h3>
      <div>
        <TruthTable theme={props.theme}>
          ...
          <TruthTableBody colSpan={3} theme={props.theme}>
            <TruthTableCurrentRow current={buttonState === 0} theme={props.theme}>
              <td>0</td><td>0</td><TruthTableOutputCell theme={props.theme}>0</TruthTableOutputCell>
      ...
      <FlexRowDiv style={{ marginTop: '50px', width: '100vw', justifyContent: 'center' }}>
        ...
        <HorizontalWire theme={props.theme} mobile={props.mobile} active={buttonState === 1 || buttonState === 2} style={{ width: props.mobile ? '50px' : '200px', marginTop: props.mobile ? '124px' : '220px' }} />
        <OutputLight theme={props.theme} mobile={props.mobile} isActive={buttonState === 1 || buttonState === 2} style={{ marginTop: props.mobile ? '112px' : '195px' }} />
      </FlexRowDiv>
    </>
  )
}
